        ...
        for char: Character in bStr.characters {
            ...
            if oldChar == char && !active {
                ...
                if findMatches(inString: String(char), usingPattern: pattern).count == 0 {
                    ...
                    let range: Range<Int> = position ..< position + 1
                    aStr = aStr.stringByReplacingOccurrencesOfString(".", withString: String(char), options: .RegularExpressionSearch, aRange: range)
